...
(defmethod cl-ds.alg.meta:make-state ((function simple-linear-regression)
                                      &rest all
...
(defmethod cl-ds.alg.meta:aggregate ((function simple-linear-regression)
                                     state
                                     element)
...
(defmethod cl-ds.alg.meta:state-result ((function simple-linear-regression)
                                   state)
  (check-type state linear-regression-state)
...
(defclass simple-linear-regression-with-error (cl-ds.alg.meta:multi-aggregation-function)
  ()
  (:metaclass closer-mop:funcallable-standard-class))
...
(defmethod cl-ds.alg.meta:make-state ((function simple-linear-regression-with-error)
                                      &rest all
...
(defmethod cl-ds.alg.meta:multi-aggregation-stages ((function simple-linear-regression-with-error)
                                                    &rest all
                                                    &key x-key y-key)
  (declare (ignore all))
  (append (cl-ds.alg.meta:multi-aggregation-stages #'simple-linear-regression
                                              :x-key x-key
                                              :y-key y-key)
          `((:regres . ,(lambda (range)
...
(defmethod cl-ds.alg.meta:aggregate ((function simple-linear-regression-with-error)
                                     state
                                     element)
...
(defmethod cl-ds.alg.meta:state-result ((function simple-linear-regression-with-error)
                                        state)
  (check-type state linear-regression-errors)
  (bind (((:slots regres count sum-of-error-squares) state)
         (er (unless (zerop count)
               (/ sum-of-error-squares count))))
    `(,@regres ,er)))
