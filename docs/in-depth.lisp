  ...
  (chunk *cl-data-structures*
    (level [section] @label{cl-ds internals}
      ...
      (level [section]
        ...
        [:text Nested data structures are nontrivial problem, when immutable behavior is expected. Because eventual copy-on-write has to happen on two levels, It is essential to be able to intercept logic at the lower level of data structure. Luckly, as described, ,(lisp) offers tools that allowed us to build elegant solution for such problem.]
        ))))
