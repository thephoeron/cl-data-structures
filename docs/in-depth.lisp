  ...
  (chunk *cl-data-structures*
    ...
    (level [section]
      ...
      (level [section]
        ...
        [:text The answer to this question is simple: cl-data-structures will call the closures only when user will query the state of the container. This will in turn destructivly change the internal container, however because of transactional semantics, changes will not leak outside. Finally, the query can be finished by forwarding call to freshly transformed internal container. At this point closures in the LAZY-BOX-CONTAINER can be discarded, as changes have been fully transfered to the state of the internal container. This creates the ilusion of fully functional interface, while minimizing the cost of copy-on-write procedure.]
        [:text To summarise: lazy evaluation of the data structures is taken care of as long as transactional variant of data structure is available.]
      ))))
